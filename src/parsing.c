/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parsing.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: amarchan <amarchan@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/23 16:05:04 by amarchan          #+#    #+#             */
/*   Updated: 2022/05/30 18:08:50 by amarchan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../headers/philo.h"

void	ft_panic(int errcode)
{
	printf("Error !\n");
	if (errcode == WRONG_NARG)
		printf("Usage :./philo number_of_philosophers time_to_die " \
		"time_to_eat " \
		"time_to_sleep [number_of_times_each_philosopher_must_eat]\n");
	else if (errcode == WRONG_ARG)
		printf("Argument is univalid");
	else if (errcode == NOT_INT)
		printf("Argument is not an integer");
	else if (errcode == OUT_INT)
			printf("Argument is not an integer");
	else if (errcode == EMPTY_STR)
		printf("Integer is bigger than 21476483647 or smaller than -2147483648");
	else if (errcode == MALLOC_FAILURE)
		printf("Memory allocation failed\n");
	exit(errcode);
}

//check if argument isn't smaller than INT_MIN or bigger than INT_MAX
static int	ft_invalid_int(long long arg)
{
	return (!(arg <= INT_MAX && arg >= INT_MIN));
}

//check if arguments are correct and if so returns 1
char **check_args(int argc, char **argv)
{
	int	i;
	
	i = 1;
	while (i < argc)
	{
		ft_strip(argv[i]);
		if (ft_strlen(argv[i]) == 0)
			ft_panic(EMPTY_STR);
		else if (ft_strlen(argv[i]) <= 11 && isinteger(argv[i]))
		{
			if (ft_invalid_int(ft_atoll(argv[i])))
				ft_panic(OUT_INT);
		}
		else 
			ft_panic(NOT_INT);
		i++;
	}
	return (argv);
}

int    ft_parse(int argc, char **argv, t_philo **philos)
{    
    (*philos)->state->set = NULL;
    (*philos)->state->set = malloc(sizeof(t_settings));
    if (!(*philos)->state->set)
        ft_panic(MALLOC_FAILURE);
    argv = check_args(argc, argv);
    (*philos)->state->set->n_philos = ft_atoi(argv[1]);
    printf("n_philos : %d\n", (*philos)->state->set->n_philos);
    (*philos)->state->set->time_to_die = ft_atoi(argv[2]);
    printf("time_to_die : %d\n", (*philos)->state->set->time_to_die);
    (*philos)->state->set->time_to_eat = ft_atoi(argv[3]);
    printf("time_to_eat : %d\n", (*philos)->state->set->time_to_eat);
    (*philos)->state->set->time_to_sleep = ft_atoi(argv[4]);
    printf("time_to_sleep : %d\n", (*philos)->state->set->time_to_sleep);
    if (argc == 6)
    {
        (*philos)->state->set->n_meals = ft_atoi(argv[5]);
        printf("n_meals : %d\n", (*philos)->state->set->n_meals);        
    }
    else
        (*philos)->state->set->n_meals = -1;
    return (0);
}
